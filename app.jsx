...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'watering-team-app';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Lắng nghe cấu hình (Tên app, danh sách thành viên)
    ...
    const unsubConfig = onSnapshot(configRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (data.members?.length > 0 && data.epochDate) {
          setStep('dashboard');
        } else {
          setStep('setup');
        }
    ...
    // Lắng nghe dữ liệu lịch trình (Hoàn thành, đổi người, ghi đè ngày)
  ...
  const saveConfig = async (newMembers, newTitle, newEpoch) => {
    if (!user) return;
    const ref = doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'main');
    try {
      await setDoc(ref, {
        title: newTitle !== undefined ? newTitle : calendarTitle,
        members: newMembers !== undefined ? newMembers : members,
        epochDate: newEpoch !== undefined ? newEpoch.getTime() : (epochDate ? epochDate.getTime() : null),
        updatedAt: Date.now()
      }, { merge: true });
  ...
  const saveSchedule = async (updates) => {
    if (!user) return;
    const ref = doc(db, 'artifacts', appId, 'public', 'data', 'schedules', 'main');
    try {
      // merges updates (completed, swaps, overrides)
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      lines.forEach((line, index) => {
        ...
        if (name && team) {
          newMembers.push({ id: Date.now().toString() + Math.random().toString(), name, team });
        }
      });
  ...
  const startDashboard = () => {
    ...
    setEpochDate(start);
    setStep('dashboard');
    saveConfig(members, calendarTitle, start);
  };
  ...
  const handleDrop = (e, targetStr) => {
    ...
    let newOverrides = { ...wateringDaysOverride };
    if (!isWateringDay(new Date(targetStr))) {
      newOverrides[targetStr] = true;
      setWateringDaysOverride(newOverrides);
    }
    saveSchedule({ swaps: newSwaps, overrides: newOverrides });
  };
      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-[2.5rem] w-full max-w-2xl overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="p-8 max-h-[60vh] overflow-y-auto custom-scrollbar bg-slate-50">
            <div className="space-y-4">
              {leaderboardData.map((m, idx) => {
                ...
                const isTop3 = idx < 3;
                let rankIcon = null;
                if (idx === 0) rankIcon = <Crown size={24} className="text-amber-500" />;
                else if (idx === 1) rankIcon = <Medal size={24} className="text-slate-400" />;
                else if (idx === 2) rankIcon = <Medal size={24} className="text-orange-500" />;

                return (
                  <div key={m.id} className={`flex items-center gap-4 p-5 rounded-3xl border transition-all hover:scale-[1.02] ${idx === 0 ? 'bg-amber-50 border-amber-200 shadow-amber-100 shadow-lg' : 'bg-white border-slate-100'}`}>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${badge.bg} ${badge.color}`}>{badge.label}</span>
                      </div>
                    </div>
          ...
          <div className="p-6 bg-white border-t border-slate-100 text-center">
             <button onClick={() => setShowLeaderboard(false)} className="bg-slate-800 text-white px-10 py-3 rounded-2xl font-black uppercase tracking-widest hover:bg-black transition-all">Đóng bảng</button>
          </div>
        </div>
      </div>
    );
      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
          <div className="bg-gradient-to-r from-emerald-500 to-teal-600 p-6 text-white relative">
            <button onClick={() => setSelectedMember(null)} className="absolute top-4 right-4 hover:bg-white/20 p-1 rounded-full transition-colors"><X size={24} /></button>
            <div className="flex items-center gap-4">
              <div className="bg-white/20 p-3 rounded-2xl"><Users size={32} /></div>
              <div>
          ...
          <div className="p-6">
            <div className="flex flex-col items-center mb-6 bg-slate-50 p-6 rounded-3xl border border-slate-100 text-center">
                <div className={`${badge.bg} ${badge.color} p-3 rounded-2xl mb-3`}><Award size={32} /></div>
                <h4 className={`text-xl font-black uppercase tracking-wide ${badge.color}`}>{badge.label}</h4>
            ...
            <h4 className="text-sm font-bold text-slate-400 mb-3 flex items-center gap-2 uppercase tracking-widest"><History size={16} /> Lịch sử tưới cây</h4>
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center">
      <div className="text-center animate-pulse">
        ...
        <p className="text-slate-400 font-black uppercase tracking-widest text-xs">Đang đồng bộ dữ liệu...</p>
      </div>
    </div>
  );
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans">
        <div className="bg-white p-8 rounded-[2rem] shadow-2xl w-full max-w-2xl border border-slate-100 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-8 opacity-5"><Leaf size={120} /></div>
          <div className="flex flex-col items-center mb-8">
            <div className="bg-emerald-500 p-4 rounded-3xl text-white shadow-lg shadow-emerald-200 mb-4"><Leaf size={32} /></div>
            <h1 className="text-3xl font-black text-slate-800">Thiết Lập Team</h1>
            <p className="text-slate-500 mt-1">Dữ liệu sẽ được lưu trữ tự động</p>
          </div>
          <div className="space-y-6 relative z-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="col-span-full">
                ...
                <input
                  type="text" value={calendarTitle} onChange={(e) => setCalendarTitle(e.target.value)}
              ...
              <div className="col-span-full border-t border-slate-100 pt-4 flex items-center justify-between">
                ...
                <label className="flex items-center gap-2 text-emerald-600 text-sm font-bold cursor-pointer hover:bg-emerald-50 px-3 py-1.5 rounded-lg transition-colors">
                  <Upload size={16} /> Import CSV (Tên,Team)
              ...
              <form onSubmit={handleAddMember} className="col-span-full flex flex-wrap md:flex-nowrap gap-2 items-start">
                <input
                  ...
                  placeholder="Tên..." className="flex-[2] w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-emerald-500 focus:outline-none"
                />
                <div className="flex-[1.5] w-full relative">
                  <div className="relative">
                    <input
                      type="text" value={newMemberTeam} onChange={(e) => { setNewMemberTeam(e.target.value); setShowTeamDropdown(true); }}
                      onFocus={() => setShowTeamDropdown(true)} placeholder="Team..."
                      className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-emerald-500 focus:outline-none pr-10"
                    />
                    <button type="button" onClick={() => setShowTeamDropdown(!showTeamDropdown)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400">
                  ...
                  {showTeamDropdown && existingTeams.length > 0 && (
                    <div className="absolute z-50 w-full mt-2 bg-white border border-slate-100 rounded-2xl shadow-xl max-h-40 overflow-y-auto custom-scrollbar">
                      {existingTeams.filter(t => t.toLowerCase().includes(newMemberTeam.toLowerCase())).map((teamName, idx) => (
                        <button key={idx} type="button" onClick={() => { setNewMemberTeam(teamName); setShowTeamDropdown(false); }} className="w-full text-left px-4 py-3 hover:bg-emerald-50 text-sm font-bold text-slate-700 transition-colors border-b border-slate-50 last:border-0">{teamName}</button>
                      ))}
                    </div>
                  )}
                </div>
                <button type="submit" className="bg-slate-800 text-white p-4 rounded-2xl hover:bg-black transition-colors shrink-0"><Plus /></button>
              </form>
            </div>
            <div className="bg-slate-50 rounded-3xl border-2 border-slate-100 p-4 max-h-48 overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-2">
              ...
              {members.map(m => (
                <div key={m.id} className="bg-white p-3 rounded-xl border border-slate-200 flex items-center justify-between group">
                  ...
                  <button onClick={() => { const u = members.filter(x => x.id !== m.id); setMembers(u); saveConfig(u); }} className="text-slate-300 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100 p-1">
                    <Trash2 size={16} />
                  </button>
                </div>
              ))}
            </div>
            <button onClick={startDashboard} className="w-full bg-emerald-500 text-white py-5 rounded-3xl font-black text-xl shadow-xl shadow-emerald-100 hover:bg-emerald-600 hover:-translate-y-1 active:translate-y-0 transition-all uppercase tracking-widest">
      ...
      <div className="max-w-7xl mx-auto">
        <header className="flex flex-col md:flex-row items-center justify-between mb-8 p-6 bg-white rounded-[2.5rem] shadow-sm border border-slate-100 gap-4">
          <div className="flex items-center gap-5">
            <div className="bg-emerald-500 p-4 rounded-3xl text-white shadow-lg shadow-emerald-100"><Globe size={32} /></div>
            <div>
          ...
          <div className="flex gap-2">
            <button onClick={() => setShowLeaderboard(true)} className="bg-amber-100 px-6 py-3 rounded-2xl font-bold text-amber-600 hover:bg-amber-200 transition-all flex items-center gap-2 border border-amber-200"><Trophy size={20} /> Bảng xếp hạng</button>
            <button onClick={() => setStep('setup')} className="bg-slate-100 px-6 py-3 rounded-2xl font-bold text-slate-600 hover:bg-emerald-50 transition-all flex items-center gap-2 border border-transparent"><Settings size={20} /> Thiết lập lại</button>
          </div>
        </header>
        ...
        <div className="bg-white rounded-[3rem] shadow-xl border border-slate-100 overflow-hidden">
          <div className="bg-slate-50/50 p-8 flex flex-col md:flex-row items-center justify-between border-b border-slate-100 gap-6">
            <div className="flex items-center gap-4">
              <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1, 1))} className="p-3 bg-white rounded-2xl shadow-sm border border-slate-200"><ChevronLeft size={24} /></button>
              <h2 className="text-2xl font-black min-w-[200px] text-center">Tháng {currentDate.getMonth() + 1}, {currentDate.getFullYear()}</h2>
              <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 1))} className="p-3 bg-white rounded-2xl shadow-sm border border-slate-200"><ChevronRight size={24} /></button>
            </div>
            <div className="flex gap-4">
               <div className="flex items-center gap-2 text-xs font-bold bg-white px-4 py-2 rounded-xl border border-slate-200"><div className="w-3 h-3 bg-red-100 border border-red-200 rounded-sm"></div> Quên tưới</div>
               <div className="flex items-center gap-2 text-xs font-bold bg-white px-4 py-2 rounded-xl border border-slate-200"><div className="w-3 h-3 bg-emerald-100 border border-emerald-200 rounded-sm"></div> Đã xong</div>
            </div>
          </div>
          ...
          <div className="p-4 md:p-8 overflow-x-auto">
            <div className="min-w-[1000px]">
              <div className="grid grid-cols-7 gap-4 mb-6">
                {['Thứ 2', 'Thứ 3', 'Thứ 4', 'Thứ 5', 'Thứ 6', 'Thứ 7', 'CN'].map(d => (<div key={d} className="text-center text-xs font-black uppercase text-slate-400 tracking-widest">{d}</div>))}
              </div>
              <div className="grid grid-cols-7 gap-4">
                {calendarData.map(cell => {
                  ...
                  return (
                    <div key={cell.dateStr} onDragOver={e => e.preventDefault()} onDrop={e => handleDrop(e, cell.dateStr)}
                      className={`relative aspect-square rounded-[2rem] border-2 p-4 flex flex-col transition-all group ${today ? 'border-emerald-400 bg-emerald-50/30 ring-8 ring-emerald-50' : 'border-slate-100 bg-white'} ${showAlert ? 'bg-red-50 border-red-200' : ''} ${!isWorkDay ? 'opacity-40 grayscale' : ''}`}
                    >
                      <div className="flex justify-between items-start mb-2">
                        <span className={`text-xl font-black ${today ? 'text-emerald-600' : 'text-slate-400'}`}>{cell.dayOfMonth}</span>
                        <button onClick={() => { const up = { ...wateringDaysOverride, [cell.dateStr]: !isWorkDay }; setWateringDaysOverride(up); saveSchedule({ overrides: up }); }} className={`p-2 rounded-xl transition-all ${isWorkDay ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}>
                        ...
                        <div className="mt-auto relative">
                          {isDone && (
                            <div className="absolute -top-12 left-1/2 -translate-x-1/2 flex flex-col items-center animate-bounce pointer-events-none whitespace-nowrap">
                              <span className="text-[10px] bg-emerald-600 text-white px-2 py-1 rounded-full font-bold shadow-lg mb-1 flex items-center gap-1"><Globe size={10} /> Cảm ơn bạn!</span>
                              <div className="text-2xl">🌱</div>
                            </div>
                          )}
                          <div draggable onDragStart={e => e.dataTransfer.setData('text', cell.dateStr)}
                            className={`p-3 rounded-2xl border shadow-sm transition-all cursor-grab ${isDone ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-200'} ${showAlert ? 'border-red-300' : ''}`}
                          >
                            ...
                            <label className="flex items-center gap-2 cursor-pointer">
                              <input type="checkbox" checked={!!isDone} 
                                onChange={() => { const up = { ...completed, [cell.dateStr]: !completed[cell.dateStr] }; setCompleted(up); saveSchedule({ completed: up }); }}
                                className="w-4 h-4 accent-emerald-500 rounded-md"
                              />
                              <span className={`text-[10px] font-black uppercase ${isDone ? 'text-emerald-600' : 'text-slate-400'}`}>{isDone ? 'Đã Xanh' : 'Chưa Tưới'}</span>
                            </label>
                          </div>
                        </div>
                      ) : <div className="mt-auto text-center py-4 text-[10px] font-black uppercase text-slate-300 tracking-widest">Ngày nghỉ</div>}
                    </div>
                  );
                })}
              </div>
          ...
          <div className="p-8 bg-emerald-900 text-white flex flex-col md:flex-row items-center justify-center gap-4 text-center">
             <Globe className="text-emerald-400 animate-spin-slow" /><p className="text-lg font-bold italic">"Cảm ơn bạn đã góp nước để hành tinh này xanh hơn"</p>
          </div>
        </div>
      </div>
      <LeaderboardModal /><HistoryModal />
      <style>{`
